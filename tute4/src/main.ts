...
import { fromEvent, interval, merge, zip, bufferCount } from "rxjs";
...
export function createRngStreamFromSource<T>(source$: Observable<T>) {
  return function createRngStream(seed: number = 0): Observable<number> {
    ...
    const randomNumberStream = source$.pipe(
      scan((prevHash: number,_) => RNG.hash(prevHash), seed),
...
function piApproximation() {
  ...
  type Data = Readonly<{
    ...
    insideDots: number; // Add additional props as you see fit
    outsideDots: number;
  }>;
...
/*
  ------------------------coding steps---------------------------
step 1 - zip the random values together, note that this returns an array so use the proper syntax
step 2 - destructure the array from zip and create x and y values
step 3 - create a new object with x and y values with color being conditional
step 4 - check if inside dots's color is green or red, and update the insideDots and outsideDots
	 since this is using scan, be sure to use the proper syntax
step 5 - approximate the pi using the number of insideDots and outside dots

 ------------------------human steps---------------------------
step 1 - create 2 random values for x and y
step 2 - create a new dot with x and y values and color being conditional
step 3 - make changes to the number of dots inside and outside
step 4 - approximate the pi using the number of insideDots and outside dots
step 5 - add the dots to the canvas
*/

  // we use zip to create an array of 2 random numbers
  const dot$ = zip(rngStream(0),rngStream(1)).pipe(
    // we destructure it and create an object with x and y properties
    map(([x,y])=>({x,y})),
    // we destructure the object and create a new object with x,y,colour properties
    map(({x,y})=>({x,y,colour: inCircle(x,y) ? "green" : "red"} as Dot)),
    // we use tap to add the dot to canvas
    tap(addDotToCanvas),
    // destructure the object and increment the number of dots
    scan(({insideDots, outsideDots},{colour}) => ({
      insideDots: colour === "green" ? insideDots + 1: insideDots, 
      outsideDots: colour === "red" ? outsideDots + 1: outsideDots,}), 

      {outsideDots:0,insideDots:0}
  ),
    // we approximate Pi using the number of dots
  map(({insideDots,outsideDots}) => ({
    piApproximation: approximatePi(insideDots, insideDots+outsideDots)}))
  )
  .subscribe(({ piApproximation}) => {
    resultInPage.innerText = String(piApproximation);
  });
}
